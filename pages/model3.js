...
export async function getStaticProps() {
  const model_3_info = {
    ...
    info_sections: [
      {
        title: 'Section 1',
        description: 'Information about Model 3',
      },
      {
        title: 'Section 2',
        description: 'More information about Model 3',
      },
    ],
  };
...
export default function Model3({ model_3_info }) {
  return (
    <div className="bg-gray-100 text-gray-800 min-h-screen flex flex-col items-center">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 flex flex-col items-center">
        <h1 className="text-4xl font-bold mb-6 text-green-600 text-center">{model_3_info.title} Details</h1>
        <section className="w-full flex flex-col md:flex-row md:gap-4">
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-between p-4 bg-white rounded-lg shadow-lg">
            {model_3_info.info_sections.map((infoSection, index) => (
